...
function generateLookupMarkup(){
 ...
 var classes = "#display-area .text-content"
...
function lookupWord(word) {
  ...
  if (word in dpd_i2h) {
    out += `<strong>${originalWord}</strong><br><ul style="line-height: 1.4em; padding-left: 15px; margin-top: 4px;">`;
    for (const headword of dpd_i2h[word]) {
      if (headword in dpd_ebts) {
        // Match headword with optional trailing number
        const match = headword.match(/^(.+?)\s(\d[\d.]*)$/);
        let formattedHeadword;
        if (match) {
          const wordPart = match[1].trim();
          const numberPart = match[2];
          // Use <sub> with styling
        ...
        out += `<li>${formattedHeadword}: ${dpd_ebts[headword]}</li>`;
      }
    }
    out += "</ul>";
  }
  ...
  if (word in dpd_deconstructor) {
    out += `<strong>${originalWord}</strong><br><ul style="line-height: 1.4em; padding-left: 15px; margin-top: 4px;">`;
    out += `<li>${dpd_deconstructor[word]}</li></ul>`;
  }
out+="</ul>"
