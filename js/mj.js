...
// --- ROADMAP / CURRICULUM SYSTEM ---
   ...
   enrollCertification() {
    ...
    this.save();
    this.renderGoals();
    this.showToast('Enrolled: Basic Meditation 1 Certification Program');
...
renderCourseList(container) {
    ...
    renderCourseDetail(container, courseId) {
        ...
        header.innerHTML = `
            ...
            <div>
                ...
                <span style="font-size:12px; color:var(--text-light)">Lesson course</span>
            </div>
        `;
    ...
    actionBtn.onclick = (e) => {
        ...
        if (isCompleted) {
            ...
            actionBtn.className = isCompleted ? 'btn btn-secondary' : 'btn';
            actionBtn.innerHTML = isCompleted ? '<i class="fas fa-check"></i> Completed' : 'Mark as Completed';
            if (isCompleted) {
    ...
    this.showToast(`Practice exercise: ${step.title}`);
}
...
concludeMeditationSession(type = 'manual') {
    ...
    if (this.meditationState.isExam) {
      ...
      `- Duration: ${minutes}m (Required 60m) ${passedDuration ? '✅':'❌'}\n` +
      `- Quality: ${qualityPct}% (Required ≥ 50%) ${passedQuality ? '✅':'❌'}\n` +
      `- Focus Level: ${avgScore.toFixed(2)} (Required ≥ 1.2) ${passedScore ? '✅':'❌'}\n` +
      `- Density: ${density.toFixed(1)} (Required ≥ 5) ${passedDensity ? '✅':'❌'}\n` +
      `- Total Mindfulness: ${totalCount} (Required ≥ 500) ${passedCount ? '✅':'❌'}\n\n` +
...
calculateGoalTier(goal) {
    ...
    // 5. Determine Tier
    if (totalScore >= 0.9) return { id: 'diamond', name: 'Diamond Class', class: 'tier-diamond', icon: 'fab fa-sketch' };
    if (totalScore >= 0.75) return { id: 'gold', name: 'Gold Class', class: 'tier-gold', icon: 'fas fa-jedi' };
    if (totalScore >= 0.5) return { id: 'silver', name: 'Silver Class', class: 'tier-silver', icon: 'fab fa-ethereum' };
...
calculateConsistencyScore(goal) {
    ...
    sortedGoals.forEach(goal => {
        ...
        if (showBadge) {
            ...
            const displayTier = isCertGoal ? { name: 'Meditation Certificate', icon: 'fas fa-graduation-cap', class: 'tier-diamond' } : tier;
...
<p style="color: var(--text-light); font-size: 13px; margin-bottom: 20px;">
    You have completed 10,000 mindfulness. Pass the final test to receive your certificate.
</p>
...
<button class="btn" style="width: 100%; background: var(--warning); color: black; font-weight: bold;" onclick="app.startExamSession('${goal.id}')">
    <i class="fas fa-play-circle"></i> Start Exam (60m)
</button>
             `;
             container.appendChild(div);
             return;
        ...
        if (dailyTarget > 0) {
            ...
            const sessionClick = isCertGoal ? `onclick="app.showToast('Cannot change certificate goal')"` : `onclick="app.setDailySessionTarget('${goal.id}')"`;
            ...
            if (isMeditation) {
                ...
                const minClick = isCertGoal ? `onclick="app.showToast('Cannot change certificate goal')"` : `onclick="app.setDailyMinMedTarget('${goal.id}')"`;
